...
class Environment:
    ...
    def save_data(self):
        np.savetxt('/home/prolee/apps/UAV_Obstacle_Avoiding_DRL-master/Dynamic_obstacle_avoidance/data_csv/start.csv', self.start, delimiter=',')
        np.savetxt('/home/prolee/apps/UAV_Obstacle_Avoiding_DRL-master/Dynamic_obstacle_avoidance/data_csv/goal.csv', self.goal, delimiter=',')
        np.savetxt('/home/prolee/apps/UAV_Obstacle_Avoiding_DRL-master/Dynamic_obstacle_avoidance/data_csv/obs_r_list.csv',self.obs_r,delimiter=',')
        for i in range(self.obs_num):
            np.savetxt('/home/prolee/apps/UAV_Obstacle_Avoiding_DRL-master/Dynamic_obstacle_avoidance/data_csv/obs{}_trace.csv'.format(i),self.path[i],delimiter=',')
    ...
    def load_model(method):
        import sys
        sys.path.append('/home/prolee/apps/UAV_Obstacle_Avoiding_DRL-master/Dynamic_obstacle_avoidance/IIFDS-{}-random_start'.format(method)) # 解决保存模型路径不是加载文件路径的情况
    ...
    def load_Q(method):
        ...
        Q_network = torch.load('/home/prolee/apps/UAV_Obstacle_Avoiding_DRL-master/Dynamic_obstacle_avoidance/IIFDS-{}-random_start/TrainedModel/dynamicCritic.pkl'.format(method), map_location=device)
        return Q_network
    ...
    def load_human(method):
        ...
        humanController = torch.load('/home/prolee/apps/UAV_Obstacle_Avoiding_DRL-master/Dynamic_obstacle_avoidance/IIFDS-{}-random_start/TrainedModel/dynamicActor_hh.pkl'.format(method), map_location=device)
...
if __name__ == "__main__":
    ...
    env = Environment(3)              # 这里用不同的数字初始化不同的动态场景
    config = Config()
    METHOD = 'DDPG'                       # 在这填写测试用的模型
    controller = env.load_model(METHOD)
    q_network= env.load_Q(METHOD)
    ...
    for step in range(500):
        ...
        if if_test_origin_ifds:
            ...
            action_m = controller(state).cpu().detach().numpy()
            action_m = transformAction(action_m, config.actionBound, config.act_dim)
            action_h = human(state).cpu().detach().numpy()
            action_h = transformAction(action_h, config.actionBound, config.act_dim)
            #计算Q值
            action_tensor = torch.as_tensor(action_m, dtype=torch.float, device=device)
            q_value=q_network(state,action_tensor)
            ...
            c_2=-(x+y)*R*np.linalg.norm(deta_J,1)**2+1
            #print(c_2)
            c_1=np.linalg.norm(deta_J,2)**2+1
            #print(c_1)
            alpha=-c_1/(2*c_2)
            #print(alpha)
            ...
            omega=1/(1+np.exp(-10000*theta))
            #print(omega)
            omega_stack.append(omega)
            #人机混合动作
            a=omega*np.array(action_m)+(1-omega)*np.array(action_h)
    ...
    # save trace
    # np.savetxt('/home/prolee/apps/UAV_Obstacle_Avoiding_DRL-master/Dynamic_obstacle_avoidance/data_csv/multipathMatrix.csv',uav_path,delimiter=',')
    # env.save_data()
    # np.savetxt('/home/prolee/apps/UAV_Obstacle_Avoiding_DRL-master/Dynamic_obstacle_avoidance/multi_csv/reward_rl.csv',reward_stack,delimiter=',')
    
    # # save action trace
    # np.savetxt('/home/prolee/apps/UAV_Obstacle_Avoiding_DRL-master/Dynamic_obstacle_avoidance/data_csv/actionTrace{}.csv'.format(METHOD),action_trace,delimiter=',')
    
    # #保存相关参数
    # # np.savetxt('/home/prolee/apps/UAV_Obstacle_Avoiding_DRL-master/Dynamic_obstacle_avoidance/multi_csv/theta_al2.csv', theta_stack, delimiter=',')
    #np.savetxt('/home/prolee/apps/UAV_Obstacle_Avoiding_DRL-master/Dynamic_obstacle_avoidance/multi_csv/omega_our.csv', omega_stack, delimiter=',')
    #np.savetxt('/home/prolee/apps/UAV_Obstacle_Avoiding_DRL-master/Dynamic_obstacle_avoidance/multi_csv/alpha.csv',alpha_stack,delimiter=',')
    # # save distance between UAV and Obstacle
    # np.savetxt('/home/prolee/apps/UAV_Obstacle_Avoiding_DRL-master/Dynamic_obstacle_avoidance/data_csv/d_list_{}.csv'.format(if_test_origin_ifds),d_list,delimiter=',')
    plt.show()
